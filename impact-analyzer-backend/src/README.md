...
Open your backend file:

services/sagemakerService.js


Replace your invoke code with this.
...
async function predictRisk(featuresArray) {
  try {
    console.log("🤖 Calling SageMaker endpoint:", process.env.SAGEMAKER_ENDPOINT);
...
🔧 Update analyzerService

Where you call SageMaker:

Change this:

const payload = {
  instances: [[files, msgLen, 0.5]]
}
